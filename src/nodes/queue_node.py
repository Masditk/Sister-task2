...
class QueueNode(BaseNode):
    def __init__(self):
        super().__init__()
        peers = [p for p in self.raft.peers] + [f"http://{self.id}:{self.port}"]
        self.ring = ConsistentHashRing(peers)
    ...
    async def apply(self, entry):
        ...
        if etype == "QUEUE_ENQ":
            ...
            queues[shard].append(payload)
            queue_enq.inc()
            queue_lag.labels(shard).set(len(queues[shard]))

        elif etype == "QUEUE_ACK":
            msg_id = entry["msg_id"]
    ...
    async def enq(self, req):
        ...
        ok = await self.raft.replicate_and_commit({
            "type": "QUEUE_ENQ",
            "shard": shard,
            "payload": data
        })
    ...
    async def deq(self, _):
        for shard, q in queues.items():
            if q:
                ...
                inflight[msg_id] = (time.time(), shard)
                return web.json_response({"msg_id": msg_id, "payload": msg})

        now = time.time()
        for msg_id, (ts, shard) in list(inflight.items()):
            if now - ts > VIS_TIMEOUT:
                queues[shard].insert(0, {"redelivered": True, "msg_id": msg_id})
                inflight.pop(msg_id, None)
